...
router.get("/:category", async (req, res) => {
  try {
    const category = req.params.category.trim(); // Ensure no extra spaces
    ...
    if (!questions.length) {
      console.log("⚠️ No questions found for category:", category);
      return res.status(404).json({ message: "No questions found in this category" });
    }

    console.log("✅ Questions found:", questions.length); // Log count
    res.json(questions);
  } catch (error) {
